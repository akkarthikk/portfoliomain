...
app.post("/recover", async (req, res) => {
  ...
  try {
    const users = await User.find(
      { email, security_answer: s_answer },
      "name email password"
    );
    if (users.length > 0) {
...
app.post("/posts", ensureAuthenticated, async (req, res) => {
  try {
    ...
    const newPost = await Post.create({ email, title, posts });
    console.log(newPost);
    res.redirect("/blog");
  } catch (error) {
    console.error(error);
...
app.get("/posts/delete/:id", ensureAuthenticated, async (req, res) => {
  try {
    ...
    const result = await Post.deleteOne({ _id: id, email });
    console.log(result);
